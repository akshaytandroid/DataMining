...
import com.akshay.dataminingdemo.R;
...
public class PieChartMain extends View implements View.OnTouchListener {
    ...
    private PieChartState state = PieChartState.WAIT;
    ...
    private List<PieChartSlice> slicesList = new ArrayList<>();

    public PieChartMain(Context context) {
        super(context);
    ...
    public PieChartMain(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        try {
            ...
            this.sliceColor = ta.getColor(R.styleable.PacPieChart_sliceDefaultColor, context.getResources().getColor(R.color.colorPrimary));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (state != PieChartState.IS_READY_TO_DRAW) {
            return;
        }
        ...
        start = PieChartState.START_INC.stateCode;

        /**
         * draw each slice
         */
        for (int i = 0; i < slicesList.size(); i++) {
            PieChartSlice item = slicesList.get(i);
            ...
            sliceName = item.label;
            Log.i(TAG, "onDraw: SliceName " + sliceName);
        ...
        state = PieChartState.IS_DRAW;
    }

    public void setValues(List<PieChartSlice> data) {
        ...
        state = PieChartState.IS_READY_TO_DRAW;
    }
    ...
    public int getColorValues(int index) {
        if (slicesList == null) {
            ...
            return ((PieChartSlice) slicesList.get(0)).color;
        else if (index > slicesList.size())
            return ((PieChartSlice) slicesList
...
//                    performClick(x, y);
    ...
    private void performSliceClick(float x, float y) {



        Log.i(TAG, "performSliceClick: Label Name is " + sliceName);

    }

    public void performClick(float x, float y) {
        Toast.makeText(context, "Co-Ordinates X= " + x + " and Y =" + y, Toast.LENGTH_SHORT).show();
    }
    ...
    public void expand() {
        final int initialHeight = PieChartMain.this.getMeasuredHeight();
        PieChartMain.this.setVisibility(View.VISIBLE);
        Animation a = new Animation() {
            @Override
            protected void applyTransformation(float interpolatedTime, Transformation t) {

                state = PieChartState.IS_READY_TO_DRAW;
                ViewHelper.setScaleX(PieChartMain.this, interpolatedTime);
                ViewHelper.setScaleY(PieChartMain.this, interpolatedTime);
                if (isRotationActivated) {
                    ViewHelper.setRotation(PieChartMain.this, 360 * interpolatedTime);
                }
                PieChartMain.this.invalidate();
            }
        ...
        a.setDuration((int) (initialHeight * 2 / PieChartMain.this.getContext().getResources()
                .getDisplayMetrics().density));
        PieChartMain.this.startAnimation(a);
    }
        ...
        Animation a = new Animation() {
            ...
            protected void applyTransformation(float interpolatedTime, Transformation t) {
                if (interpolatedTime == 1) {
                    PieChartMain.this.setVisibility(View.GONE);
                } else {
                    state = PieChartState.IS_READY_TO_DRAW;
                    ViewHelper.setScaleX(PieChartMain.this, 1 - interpolatedTime);
                    ViewHelper.setScaleY(PieChartMain.this, 1 - interpolatedTime);
                    if (isRotationActivated) {
                        ViewHelper.setRotation(PieChartMain.this, 360 * (1 - interpolatedTime));
                    }
                    PieChartMain.this.invalidate();
                }
            }
     ...
     * @param state
     */
    public void setState(PieChartState state) {
        this.state = state;
    }
